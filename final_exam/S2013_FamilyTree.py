...
class Family(object):
    ...
    def cousin(self, a, b):
        ...
        """
        an, bn = self.names_to_nodes[a], self.names_to_nodes[b]
        aps, bps = self.get_ancestors(an), self.get_ancestors(bn)
        if not self.is_cousin(a, b):
            #print aps, bps
            return -1, abs(len(aps) - len(bps)) 
        else:
            # get list of a and b's ancestors
            #print aps, '\n',  bps
            # find the short distance to there common ancestor
            i = j = 0
            for i in range(len(aps)):
                for j in range(len(bps)):
                    if aps[i] == bps[j]:
                        ...
                        type_ = min(i, j)
                        rmv = abs(i - j)
            # find the  difference of the distances
            return type_, rmv

    def is_cousin(self, a, b):
        '''Return True if a and b are cousins, eles False.
        ...
        if a == b:
            return False
        elif (str(a) in self.get_ancestors(self.names_to_nodes[b]) or
              str(b) in self.get_ancestors(self.names_to_nodes[a])):
        #elif self.is_parent(str(a), str(b)) and \
        #     self.is_child(str(a), str(b)):
            return False
        else:
            return True

    def get_ancestors(self, x):
        if str(x) == str(self.root):
            return []
        else:
            ancestors = []
            p = x.get_parent()
            ...
            while p!= self.root:
                p = p.get_parent()
                ancestors.append(p)
            return [str(m) for m in ancestors]

    #def get_cousin(self, aps, bps):
